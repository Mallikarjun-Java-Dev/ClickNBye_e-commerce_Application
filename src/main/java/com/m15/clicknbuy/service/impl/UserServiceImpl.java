...
public class UserServiceImpl implements UserService {
	...
	public String checkout(HttpSession session, Principal principal, ModelMap map) {
		...
		if (items.isEmpty()) {
			...
			boolean paymentConfigured = razorpayKeyId != null && razorpayKeySecret != null && !razorpayKeyId.isBlank()
					&& !razorpayKeySecret.isBlank() && !razorpayKeyId.startsWith("your-")
					&& !razorpayKeySecret.startsWith("your-");
			map.put("paymentEnabled", paymentConfigured);

			if (!paymentConfigured) {
				map.put("error", "Online payment is not configured. You can place this order using Cash on Delivery.");
			...
			try {
				...
				map.put("paymentEnabled", false);
				map.put("error", "Payment initialization failed. You can still place this order using Cash on Delivery.");
				return "checkout.html";
			}
		}
	}
	...
	@Override
	public String placeCodOrder(String address, Principal principal, HttpSession session, ModelMap map) {
		if (principal == null) {
			return "redirect:/login";
		}
		if (address == null || address.isBlank()) {
			session.setAttribute("error", "Please provide a delivery address.");
		...
		order.setDeliveryAddress(address);
		order.setRazorpayOrderId("COD-" + System.currentTimeMillis());
		order.setRazorpayPaymentId("NA");
		order.setRazorpaySignature("NA");
		...
		session.setAttribute("success", "Order placed successfully with Cash on Delivery.");
		return "redirect:/user/orders";
	}
